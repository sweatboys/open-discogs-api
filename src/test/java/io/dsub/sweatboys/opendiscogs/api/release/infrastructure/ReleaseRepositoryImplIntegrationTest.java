...
public class ReleaseRepositoryImplIntegrationTest extends AbstractDatabaseIntegrationTest {
  ...
  private List<String> prepareVideos() {
    return IntStream.rangeClosed(1, 5)
        ...
        .peek(url -> client.sql(
                "INSERT INTO release_video VALUES (1, 'test_description', 'test_title', :url, :hash)")
  ...
  private List<Artist> prepareArtists() {
    ...
    for (Artist artist : artists) {
      ...
      client.sql("INSERT INTO release_artist VALUES (1, :id)").bind("id", artist.getId()).then()
          .block();
      if (artist == artists.get(2)) {
        client.sql("INSERT INTO release_credited_artist VALUES (1,:id,333,'test_role')")
