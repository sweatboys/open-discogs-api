  ...
  void getLabelReleasesReturnsAllReleases() {
    ...
    Flux.range(1, 2)
        .flatMap(i -> databaseClient
            .sql(
                "INSERT INTO release_format (release_id, format_hash, description) VALUES (:i,:i,'test-description')")
            .bind("i", i)
            .then())
        .blockLast();
    Flux.range(1, 2)
        .flatMap(i -> databaseClient
            .sql(
                "INSERT INTO label_release (label_id, release_id, category_notation) VALUES (:i,:i,'test-catno')")
            .bind("i", i)
            .then())
        .blockLast();
