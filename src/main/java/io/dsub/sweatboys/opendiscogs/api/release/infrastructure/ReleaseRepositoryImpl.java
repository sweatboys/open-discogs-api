...
import io.dsub.sweatboys.opendiscogs.api.release.dto.*;
...
public class ReleaseRepositoryImpl implements ReleaseRepository {
    ...
    public Mono<ReleaseDetailDTO> getById(Long id) {
        return Mono.zip(delegate.findById(id), getGenres(id), getStyles(id), getArtists(id), getLabels(id), getContracts(id), getFormats(id), getVideos(id))
    ...
    private Mono<List<ReleaseFormatDTO>> getFormats(Long releaseId) {
        return Flux.from(ctx.select(RELEASE_FORMAT.NAME, RELEASE_FORMAT.QUANTITY, RELEASE_FORMAT.DESCRIPTION)
                        .from(RELEASE_FORMAT)
    ...
    private Mono<List<ReleaseArtistDTO>> getArtists(Long releaseId) {
        ...
        var relatedArtists = select(asterisk()).from(releaseArtists).unionAll(creditedArtists).asTable("artist");

        return Flux.from(ctx.select(field("id"), field("name"), field("string_agg(distinct trim(artist.role), ',')", String.class).as("role"))
                        .from(relatedArtists)
